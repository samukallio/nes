...
static inline u16 SweepTargetPeriod(apu_pulse& Pulse, bool NegateOnesComplement)
{
	// Compute sweep unit target time period and muting flag.
	i32 Period = Pulse.TimerPeriod;
	i32 PeriodDelta = Period >> Pulse.SweepShift;
...
void StepAPU(machine& Machine)
{
	...
	// Update DMC channel.
	{
		...
		if (D.SampleBufferEmpty && D.SampleTransferCounter > 0) {
			...
			D.SampleBuffer = Read(Machine, D.SampleTransferPointer);
